...
from mapilio_kit.components  import point as P_exe
from mapilio_kit.components import video_blender as sample_parser, basics_blender as parser
...
def gps_from_stream(
...
) -> T.Generator[P_exe.PointWithFix, None, None]:
    ...
    if gpsf is not None:
        gpsf_value = P_exe.GPSFix(gpsf[0][0])
    else:
        gpsf_value = None
...
def _find_first_gps_stream(stream: T.Sequence[KLVDict]) -> T.List[P_exe.PointWithFix]:
    sample_points: T.List[P_exe.PointWithFix] = []
...
def _extract_points_from_samples(
    ...
    points_by_dvid: T.Dict[int, T.List[P_exe.PointWithFix]] = {}
...
def extract_points(fp: T.BinaryIO) -> T.Optional[T.List[P_exe.PointWithFix]]:
    """
...
def parse_gpx(path: pathlib.Path) -> T.List[P_exe.PointWithFix]:
